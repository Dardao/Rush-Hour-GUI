...
def main():
    ...
    ap.add_argument('--stop-at-experience',action='store_true',help='Stop when greedy reproduces every retained self-experience cost; no app references')
    ...
    for epoch in range(0,a.epochs+1):
        ...
        if epoch%5==0 or epoch==1 or epoch==a.epochs or epoch==a.stop_after:
            ...
            if a.stop_at_experience and all(s=='solved' and display_moves(p,path,True)==display_moves(p,paths[p.name],True) for p,path,s in zip(ps,new_paths,status)):
                print('All self-discovered best costs reproduced by neural greedy.',flush=True);break
